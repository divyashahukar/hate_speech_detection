...
def predict():
	int_features = [object(x) for x in request.form.values()]  # Request for data values
	final_features = [np.array(int_features)]  # convert into aaray
	prediction = model.predict(final_features) # Predict
	if prediction ==0:
		return render_template('index.html',prediction_text="Neither offensive nor non-offensive".format(prediction))
	else:
		return render_template('index.html',prediction_text="Offensive Language".format(prediction))
